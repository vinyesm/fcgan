...
% 2009-07-21: Added ability to auto-get GPLed source
...
[mfiledir mfilestr] = fileparts(mfilename('fullpath'));
if ~exist(fullfile(mfiledir, srcdir,'license.txt'))
    % it's nasty to repeat code here, but
    % I'm in a hurry because of this stupid issue
    curdir = pwd;
    cd (mfiledir);
    cd (srcdir);
    try
...
olddir
try
    mexcmd = sprintf('mex %s -O -I%s -I%s %s',...
        mexopts, headerdir, srcdir, [files{:}]);
    mexcmd
    eval(mexcmd);
    cd(olddir);
catch
